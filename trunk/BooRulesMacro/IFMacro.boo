...
class IFMacro(UsefulMacroBase):
	
	static final Usage = "Usage: IF:"
	...
	override def Expand(macro as MacroStatement):
		...
		if parent.Name.Equals("RULE", System.StringComparison.InvariantCultureIgnoreCase):
			...
			if parent.ContainsAnnotation(Consts.AGGREGATORNAME):
				rulename = parent[Consts.AGGREGATORNAME] as string
			else:
				rulename = SafeIdentifierName(opt(parent, "Label", L("Aggregator${Helpers.Counter}")).ToString())
				parent[Consts.AGGREGATORNAME] = rulename
		
			for stmt in macro.Block.Statements:
				if stmt isa ExpressionStatement:
					AddAggregators(block, stmt as ExpressionStatement, rulename)
			
		elif parent.Name.Equals("OVERRIDE", System.StringComparison.InvariantCultureIgnoreCase):
			Errors.Add(CompilerErrorFactory.CustomError(macro.LexicalInfo, "An Override does not take any conditions."))
			return null
	...
	def AddProductions(block as Block, st as ExpressionStatement, rulename as string):
		...
		if stmt:
			...
			mie.Target = CreateReferenceExpression("${rulename}.AddConditionToLHS")
			
			block.Add(mie)

	def AddAggregators(block as Block, st as ExpressionStatement, rulename as string):
		stmt = st.Expression as MethodInvocationExpression
		if stmt:
			for i in range(stmt.Arguments.Count):
				refarg = stmt.Arguments[i] as ReferenceExpression
